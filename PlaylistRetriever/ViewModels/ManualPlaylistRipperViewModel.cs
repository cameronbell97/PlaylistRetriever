...
{
    internal class ManualPlaylistRipperViewModel : ViewModelBase
    {
        // Declarations //
        ...
        public ManualPlaylistRipperViewModel()
        {
            ...
            spotifyClientOld = new SpotifyClientOld();
        ...
        internal async Task LoadPlaylistFromIDAsync()
        {
            ...
            try
            {
                ...
                string playlistIDExtraction = SpotifyClientOld.GetPlaylistIDFromUrl(PlaylistID);

                // Gets the API URL from the playlist ID
                await LoadPlaylistFromUrl(SpotifyClientOld.GetPlaylistApiUrlFromID(playlistIDExtraction));
            }
            catch (Exception ex)
            {
        ...
        public async Task<bool> GetPlaylistAsync(string playlistApiUrl)
        {
            ...
            string accessToken = await Task.Run(() => SpotifyClientOld.GetAccessTokenAsStringAsync(ApiAccessKey));

            spotifyClientOld ??= new SpotifyClientOld(); // If spotifyClient is null, instantiate

            // Load HTTP Info
            spotifyClientOld.LoadNewHttpInfo(playlistApiUrl, accessToken);

            // Get playlist from Spotify & convert it into Playlist Object
            PlaylistLoadStatus = "Retrieving playlist from Spotify...";
            SpotifyPlaylist playlist = await Task.Run(() => spotifyClientOld.GetPlaylistFromUrlAsync(playlistApiUrl, SPOTIFY_PLAYLIST_COLUMNS_TO_RETRIEVE));
