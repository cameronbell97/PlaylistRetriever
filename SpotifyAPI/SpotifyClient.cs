...
namespace SpotifyAPI
{
    public class SpotifyClient
    {
        ...
        public static string GetPlaylistApiUrlFromID(string playlistID)
        {
            return $"{SPOTIFY_API_URL}{SPOTIFY_API_PLAYLIST_URL}/{playlistID}";
        }
        ...
        public static async Task<SpotifyPlaylist> ConvertPlaylistFromJsonAsync(string playlistJson)
        {
            SpotifyPlaylist playlist = JsonConvert.DeserializeObject<SpotifyPlaylist>(playlistJson);
            return playlist;
        }
        ...
        {
            if (response.IsSuccessStatusCode)
                return await ConvertPlaylistFromJsonAsync(response.Content.ReadAsStringAsync().Result);
            return null;
        }

        // Protected & Private Methods //

        // TODO : Make more generic <T>
        /// <summary>
        /// 
        /// </summary>
        /// <param name="sTracksJson"></param>
        /// <returns></returns>
        private static async Task<SpotifyPaging<SpotifyPlaylistTrack>> ConvertTracksAsync(string sTracksJson)
        {
            SpotifyPaging<SpotifyPlaylistTrack> tracks = JsonConvert.DeserializeObject<SpotifyPaging<SpotifyPlaylistTrack>>(sTracksJson);
            return tracks;
        }

        public static string GetEncodedAPIKey(string clientID, string clientSecret)
        {
            return System.Convert.ToBase64String(System.Text.Encoding.UTF8.GetBytes($"{clientID}:{clientSecret}"));
        }
    }
}
